...
class ActivationBuffer:
    def __init__(self, shape, dtype, storage, path=None):
        ...
        if storage == "ram":
            ...
            bytes_required = int(np.prod(self.shape)) * np.dtype(np_dtype).itemsize
            try:
                with open(path, "wb") as handle:
                    os.posix_fallocate(handle.fileno(), 0, bytes_required)
            except AttributeError:
                pass
            except OSError as exc:
                raise RuntimeError(
                    f"Not enough disk space for activation cache {path} "
                    f"({bytes_required / (1024 ** 3):.1f} GiB required). "
                    "Use --activation_storage ram or free disk space."
                ) from exc
        ...
        while True:
            ...
            if collect_hessian:
                collector = HessianCollector(
                    module_names,
                    [module for _name, module in get_named_modules(layer, model_type)],
                    device,
...
def forward_layer(model_type, layer, layer_idx, inps, outs, forward_kwargs, collect_hessian, args, devices):
    module_names = [name for name, _module in get_named_modules(layer, model_type)]
...
def load_squeezellm_init_layer(initial_lut, layer_idx, module_names=None):
    ...
    parsed = {}
    names = module_names or list(init_layer.keys())
    for name in names:
...
def quantize_luts(args):
    ...
    for layer_idx in layer_ids:
        ...
        module_names = list(model_layer.keys())
        init_layer = None
        if args.initial_lut:
