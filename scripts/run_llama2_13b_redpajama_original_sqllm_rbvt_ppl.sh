...
JOB_NAME="${JOB_NAME:-llama2-13b-sqllm-rbvt}"
...
MODEL_LABEL="${MODEL_LABEL:-llama2_13b}"
...
RBVT_LAMBDA="${RBVT_LAMBDA:-5.0}"
RBVT_TOPK="${RBVT_TOPK:-0}"
RBVT_BUDGET_P="${RBVT_BUDGET_P:-0.02}"
RBVT_TARGET_RATIO="${RBVT_TARGET_RATIO:-0.5}"
RBVT_MSE_GUARD="${RBVT_MSE_GUARD:-1}"
RBVT_ROW_CHUNK="${RBVT_ROW_CHUNK:-1024}"
RBVT_GAP_FLOOR="${RBVT_GAP_FLOOR:-1e-8}"
RBVT_MODEL_DTYPE="${RBVT_MODEL_DTYPE:-default}"
...
PPL_TARGETS="${PPL_TARGETS:-squeezellm rbvt_squeeze}"
...
OUTPUT_ROOT="${OUTPUT_ROOT:-outputs/llama2_13b_3bit_redpajama_original_sqllm_rbvt}"
CACHE_ROOT="${CACHE_ROOT:-cache/llama2_13b_3bit_redpajama_original_sqllm_rbvt}"
...
elif [[ "${ATTN_IMPLEMENTATION}" == "flash_attention_2" ]] && ! flash_attn_ready; then
  if [[ "${REQUIRE_FLASH_ATTN:-0}" == "1" ]]; then
    echo "[${JOB_NAME}] flash-attn is required but not importable in ${PYTHON_BIN}" >&2
    exit 1
  fi
  echo "[${JOB_NAME}] flash-attn is not importable; falling back to sdpa" >&2
  ATTN_IMPLEMENTATION="sdpa"
fi
...
RBVT_CKPT="${RBVT_CKPT:-${PACK_DIR}/${MODEL_LABEL}_rbvt_squeeze_w${BIT}.pt}"
...
RBVT_MSE_GUARD_ARGS=()
if [[ "${RBVT_MSE_GUARD}" == "1" || "${RBVT_MSE_GUARD}" == "true" || "${RBVT_MSE_GUARD}" == "TRUE" ]]; then
...
auto_gpu_devices() {
  ...
  local query
  query="$(nvidia-smi --query-gpu=index,memory.free,utilization.gpu --format=csv,noheader,nounits 2>/dev/null || true)"
  if [[ -z "${query}" ]]; then
    echo ""
    return
  fi
  ...
  while IFS=, read -r idx free util; do
    ...
    if (( free >= min_free_mb && util <= GPU_MAX_UTIL )); then
      chosen+=("${idx}")
    fi
  done <<< "${query}"
