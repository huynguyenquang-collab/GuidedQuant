...
from squeezellm.model_parse import get_layers, get_named_modules, parse_model


def resolve_model_dtype(value, device):
    if value == "auto":
        return "auto"
    if value == "float16":
...
def collect_fisher_group(model, layers, model_type, layer_ids, tokens, limit, args, device):
    ...
    try:
        for layer_idx in pending_layer_ids:
            modules = get_named_modules(layers[layer_idx], model_type)
...
def collect_fisher_group_with_retry(model, layers, model_type, layer_ids, tokens, limit, args, device):
    try:
        collect_fisher_group(model, layers, model_type, layer_ids, tokens, limit, args, device)
        ...
        collect_fisher_group_with_retry(model, layers, model_type, left, tokens, limit, args, device)
        collect_fisher_group_with_retry(model, layers, model_type, right, tokens, limit, args, device)
...
def collect_fisher(args):
    ...
    model = load_causal_lm(
        args.model,
        torch_dtype=resolve_model_dtype(args.model_dtype, args.device),
    ...
    for group in layer_groups(layer_ids, args.layers_per_pass):
        collect_fisher_group_with_retry(model, layers, model_type, group, tokens, limit, args, device)
...
def main():
    ...
    parser.add_argument(
        "--model_dtype",
        default="default",
        choices=["default", "auto", "float16", "bfloat16", "float32"],
        help="Model dtype for Fisher collection. Use bfloat16 for Qwen/Gemma-style models to avoid fp16 overflow.",
    )
